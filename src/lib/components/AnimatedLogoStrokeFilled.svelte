<script lang="ts">
  ...
  onMount(() => {
    ...
    const paths = document.querySelectorAll("#bluebell_stroke_fill #himmelblau > path") as NodeListOf<SVGPathElement>;
    ...
    // 📍 Animate i-dot:
    const iDot = document.querySelector("#bluebell_stroke_fill #i-dot") as SVGPathElement;
    ...
    // 🌿 Animate Flower Stem
    const stem = document.querySelector("#bluebell_stroke_fill #stem") as SVGPathElement;
    ...
    tl.to(
      "#bluebell_stroke_fill #h_leaf",
      { rotation: -14, duration: 0.4, ease: "power2.out", transformOrigin: "0% bottom" },
      "<"
    );

    // 🔵🔔 Animate Bluebell Stroke:
    const bluebell = document.querySelector("#bluebell_stroke_fill #bluebell") as SVGPathElement;
